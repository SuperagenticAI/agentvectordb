...
from agentvectordb.exceptions import EmbeddingError, InitializationError, SchemaError

from .conftest import VECTOR_DIMENSION_TEST  # Import from conftest
...
def test_initialization_with_custom_schema(test_db_path, test_embedding_function, custom_schema_type):
    ...
    retrieved = memory.get_by_id(
        entry_id, select_columns=["id", "content", "custom_field_string", "custom_field_int", "custom_field_list"]
...
def test_initialization_no_dimension_info(test_db_path):
    with pytest.raises(
        InitializationError, match="vector_dimension must be provided if not using an embedding_function"
    ):
...
def test_add_missing_vector_and_ef_incapable(test_db_path):
    ...
    mem_no_ef = AgentMemory(
        test_db_path, "no_ef_table_add", vector_dimension=VECTOR_DIMENSION_TEST, recreate_table=True
    )
...
def test_query_with_filters(agent_memory_instance: AgentMemory, get_embedding_func_for_test):
    vecs = generate_distinct_vectors(3, get_embedding_func_for_test)
    agent_memory_instance.add(
        content="Item A", vector=vecs[0], type="Type1", importance_score=0.8, tags=["tagA", "common"]
    )
    agent_memory_instance.add(
        content="Item B", vector=vecs[1], type="Type2", importance_score=0.5, tags=["tagB", "common"]
    )
    ...
    results_importance = agent_memory_instance.query(
        query_vector=query_vec, k=1, filters={"importance_score": {"$gte": 0.85}}
    )
...
def test_timestamp_last_accessed_on_get_by_id(
    agent_memory_instance_ts_update: AgentMemory, get_embedding_func_for_test
):
    memory = agent_memory_instance_ts_update
    entry_id = memory.add(
        content="Test get_by_id timestamp",
        vector=get_embedding_func_for_test(["ts test"])[0],
        timestamp_last_accessed=None,
    )
...
def test_delete_by_filter(agent_memory_instance: AgentMemory, get_embedding_func_for_test):
    ...
    agent_memory_instance.add(
        content="Another Delete type A", vector=get_embedding_func_for_test(["del A2"])[0], type="TypeA"
    )
...
def test_prune_memories(agent_memory_instance_ts_update: AgentMemory, get_embedding_func_for_test):
    ...
    mem.add(
        content="Prune Candidate 1",
        vector=vecs[0],
        timestamp_created=now - 86400 * 30,
        timestamp_last_accessed=now - 86400 * 20,
        importance_score=0.1,
    )
    # Entry 2: Recent, but very low importance
    mem.add(
        content="Prune Candidate 2",
        vector=vecs[1],
        timestamp_created=now - 86400 * 1,
        timestamp_last_accessed=now - 86400 * 1,
        importance_score=0.05,
    )
    # Entry 3: Old, but high importance
    mem.add(
        content="Keep Candidate 1",
        vector=vecs[2],
        timestamp_created=now - 86400 * 30,
        timestamp_last_accessed=now - 86400 * 1,
    ...
    mem.add(
        content="Keep Candidate 2",
        vector=vecs[3],
        timestamp_created=now - 86400 * 5,
        timestamp_last_accessed=now - 3600,
    ...
    pruned_actual_c1 = mem.prune_memories(
        max_age_seconds=86400 * 25, min_importance_score=0.2, filter_logic="AND", dry_run=False
    )
