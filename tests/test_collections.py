...
def test_collection_query_with_filter_sql(sync_collection: AgentMemoryCollection, get_embedding_vec):
    sync_collection.add(
        content="Log: User A login.",
        vector=get_embedding_vec("User A login"),
        type="log",
        importance_score=0.3,
        metadata={"user": "A"},
    )
    sync_collection.add(
        content="Log: User B activity.",
        vector=get_embedding_vec("User B activity"),
        type="log",
        importance_score=0.7,
        metadata={"user": "B"},
    )
    sync_collection.add(
        content="Alert: High CPU.", vector=get_embedding_vec("High CPU alert"), type="alert", importance_score=0.9
    )

    # Filter by type and importance
    results = sync_collection.query(query_text="system logs", k=1, filter_sql="type = 'log' AND importance_score > 0.5")
    ...
    if results_meta:  # Only assert if query was successful and results returned
        ...
        print(
            "Warning: Metadata SQL filter for 'metadata.user = A' returned no results. Check LanceDB SQL syntax for metadata."
        )
...
def test_collection_prune_memories(sync_collection_ts_update: AgentMemoryCollection, get_embedding_vec):
    ...
    id1 = coll.add(
        content="Prune old low",
        vector=get_embedding_vec("p1"),
        timestamp_created=now - 86400 * 10,
        timestamp_last_accessed=now - 86400 * 5,
        importance_score=0.1,
    )
    # Recent, but very low importance
    id2 = coll.add(
        content="Prune recent low",
        vector=get_embedding_vec("p2"),
        timestamp_created=now - 86400 * 1,
        importance_score=0.05,
    )
    # Old, but high importance, accessed recently
    id3 = coll.add(
        content="Keep old high",
        vector=get_embedding_vec("p3"),
        timestamp_created=now - 86400 * 10,
...
def test_collection_with_custom_schema(sync_store, test_embedding_function, custom_schema_fixture, get_embedding_vec):
    ...
    entry_id = coll.add(
        content="Custom schema entry",
        vector=get_embedding_vec("custom"),
        custom_text="my custom text",
        custom_int=42,
        custom_field_list=["a", "b"],
    )
    retrieved = coll.get_by_id(
        entry_id, select_columns=["custom_text", "custom_int", "custom_field_list"]
