...
from agentvectordb import AgentMemoryCollection, AgentVectorDBStore, AsyncAgentVectorDBStore
...
def test_store_get_or_create_collection(sync_store: AgentVectorDBStore, test_embedding_function):
    coll_name = "test_coll_1"
    collection1 = sync_store.get_or_create_collection(name=coll_name, embedding_function=test_embedding_function)
    ...
    collection2 = sync_store.get_or_create_collection(name=coll_name, embedding_function=test_embedding_function)
...
def test_store_delete_collection(sync_store: AgentVectorDBStore, test_embedding_function):
    coll_name = "to_delete"
...
def test_store_get_collection_limitations(sync_store: AgentVectorDBStore, test_embedding_function):
    ...
    new_store_instance = AgentVectorDBStore(db_path=sync_store.db_path)
...
async def test_async_store_initialization(unique_test_db_path):
    async_store = AsyncAgentVectorDBStore(db_path=unique_test_db_path)
