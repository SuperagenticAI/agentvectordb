...
async def async_collection(async_store: AsyncAgentVectorDBStore, test_embedding_function) -> AsyncAgentMemoryCollection:
    ...
    return await async_store.get_or_create_collection(
        name="default_async_collection", embedding_function=test_embedding_function, recreate=True
    )


@pytest.fixture
async def async_collection_ts_update(
    async_store: AsyncAgentVectorDBStore, test_embedding_function
