...
from lancedb.pydantic import LanceModel # For type hinting if using LanceModel based schemas
...
class AgentMemory:
    ...
    def _ensure_table_exists(self):
        ...
        try:
            if self.table_name not in self.db.table_names():
                print(f"Table '{self.table_name}' not found. Creating new table with schema {self.DynamicSchema.__name__}.")
                ...
                if self.embedding_function:
                    ...
                    if hasattr(self.embedding_function, 'source_column') and \
                       hasattr(self.embedding_function, 'generate'):
                        ...
                        print("Warning: Provided embedding_function doesn't conform to expected LanceDB EF structure for auto-config. Manual vector provision or ensure EF is LanceDB native.")
    ...
    def _prepare_data_for_add(self, data_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # If no vector and no EF capable of generating from 'content' (or other source_column)
            ...
            if not hasattr(self.embedding_function, 'source_column') or \
               not data_dict.get(self.embedding_function.source_column()):
                 raise EmbeddingError(
    ...
    def add(
        ...
        **kwargs: Any # Memory entry fields as keyword arguments
        ...
        """
        if not self.table: raise InitializationError("Table not initialized.")
    ...
    def _update_last_accessed(self, entry_ids: List[str]):
        ...
        try:
            ...
            self.table.update(
                values={"timestamp_last_accessed": time.time()},
    ...
    def query(
        ...
        if query_vector and len(query_vector) != self._vector_dimension:
             raise SchemaError(f"Query vector has dimension {len(query_vector)}, but table expects {self._vector_dimension}")
    ...
    def get_by_id(self, entry_id: str, select_columns: Optional[List[str]] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not results_df.empty:
                ...
                elif not select_columns and "vector" in entry_data: # Default: exclude vector for get_by_id unless include_vector becomes an arg
                    entry_data.pop('vector')
    ...
    def count(self, filters: Optional[Dict[str, Any]] = None, filter_sql: Optional[str] = None) -> int:
        ...
        try:
            if final_filter_sql:
                ...
                if hasattr(self.table, 'count_rows') and callable(getattr(self.table, 'count_rows')):
    ...
    def reflect_and_summarize(
        ...
        original_memories = self.query(
            query_vector=query_vector, query_text=query_text, k=k_to_retrieve,
            filters=query_filters, filter_sql=query_filter_sql,
        ...
        if not summary_content or summary_vector is None: # summary_vector can be empty list if dimension is 0 (unlikely)
            print("Summarization callback did not return valid content or vector.")
            return None
        if len(summary_vector) != self._vector_dimension:
             raise SchemaError(f"Summary vector from callback has dimension {len(summary_vector)}, expected {self._vector_dimension}")


        original_memory_ids = [mem['id'] for mem in original_memories if 'id' in mem]

        new_memory_data = {
            "content": summary_content, "vector": summary_vector,
            "type": new_memory_type, "source": new_memory_source,
