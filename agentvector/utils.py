...
def _parse_condition(key: str, value: Any) -> str:
    ...
    if isinstance(value, dict):
        ...
        if isinstance(op_val, list):
            if not op_val:
                if op in ("$in", "$has_any", "$has_all"): return "1 = 0" # False
                if op == "$nin": return "1 = 1" # True
        ...
        if op in op_map: return f"{safe_key} {op_map[op]} {op_val_sql}"
        if op == "$in": return f"{safe_key} IN ({list_vals_sql})"
        if op == "$nin": return f"{safe_key} NOT IN ({list_vals_sql})"
        if op == "$startswith": return f"STARTSWITH({safe_key}, {op_val_sql})"
        if op == "$endswith": return f"ENDSWITH({safe_key}, {op_val_sql})"
        if op == "$contains": return f"list_contains({safe_key}, {op_val_sql})"
        if op == "$has_any": return f"array_has_any({safe_key}, [{list_vals_sql}])"
        if op == "$has_all": return f"array_has_all({safe_key}, [{list_vals_sql}])"
...
def build_filter_sql(filters: Dict[str, Any]) -> str:
    ...
    for key, value in filters.items():
        ...
        if op_key_lower == "$and":
            if not isinstance(value, list): raise ValueError("$and requires a list.")
            ...
            if not isinstance(value, list): raise ValueError("$or requires a list.")
            sub_clauses = [build_filter_sql(cond) for cond in value if cond]
            if sub_clauses_filtered := [sc for sc in sub_clauses if sc]:
                logical_ops_clauses.append(f"({ ' OR '.join(sub_clauses_filtered) })")
        elif op_key_lower == "$not":
            if not isinstance(value, dict) or not value: raise ValueError("$not requires a non-empty dict.")
            if not_clause := build_filter_sql(value):
                 logical_ops_clauses.append(f"NOT ({not_clause})")
        else:
            try: conditions.append(_parse_condition(key, value))
            except ValueError as e: print(f"Warning: Skipping filter for key '{key}': {e}")
